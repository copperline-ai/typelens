...
type Section = "connections" | "api-keys" | "connector" | "theme";

const sections: { id: Section; label: string }[] = [
  { id: "connections", label: "Connections" },
  { id: "api-keys", label: "API Keys" },
  { id: "connector", label: "Claude Team Connector" },
...
function ConnectorSection() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <h1 className="text-2xl font-semibold">Claude Team Connector</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Use this endpoint in Claude Team to connect your active TypeLens profile over MCP SSE.
        </p>
      </div>
      ...
      <div className="rounded-lg border p-4 space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          {activeProfile
            ...
            : "No active connection. Select one in the Connections section first."}
        </p>
        <Button
          onClick={generateToken}
          disabled={!activeProfile || tokenState.status === "loading" || mcpEnabled === false}
          size="sm"
        ...
        {tokenState.status === "success" && (
          <p className="text-xs text-muted-foreground">
            Token expires {new Date(tokenState.expiresAt).toLocaleDateString()}.
          </p>
        )}
      </div>
      ...
      <div className="rounded-lg border p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">Connector URL</span>
        </div>
        <div className="flex items-center gap-2 rounded-md bg-muted px-3 py-2 font-mono text-xs">
          <span className="flex-1 truncate">{connectorUrl}</span>
          {tokenState.status === "success" && <CopyButton text={connectorUrl} />}
        </div>
        <p className="text-xs text-muted-foreground">
          In Claude Team: Settings {"->"} Integrations {"->"} Add MCP server, then paste this URL.
        </p>
      </div>
    </div>
  );
